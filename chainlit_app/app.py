...
def analyze_intent_rule_based(message: str) -> Dict:
    ...
    greeting_patterns = ["hello", "hi ", "hey", "greetings", "morning", "afternoon", "evening"]
    if any(pattern in message_lower for pattern in greeting_patterns):
        result.update({"intent": "greeting", "confidence": 0.9})
    
    menu_patterns = ["menu", "what do you have", "what do you offer", "show me", "what can i order"]
    if any(pattern in message_lower for pattern in menu_patterns):
        result.update({"intent": "menu", "confidence": 0.85, "navigate_to": "menu"})
    
    menu_items_global = ["coffee", "latte", "espresso", "cappuccino", "mocha", "tea", "sandwich"]
    found_items = [item for item in menu_items_global if item in message_lower]
    if found_items:
        result["entities"]["menu_items"] = found_items
        if "order" in message_lower or any(word in message_lower for word in ["get", "have", "want"]):
            result.update({"intent": "order", "confidence": 0.8})
    
    order_patterns = ["order", "buy", "purchase", "get me", "i'd like", "i want"]
    if any(pattern in message_lower for pattern in order_patterns):
        result.update({"intent": "order", "confidence": 0.85, "navigate_to": "orders"})
    
    status_patterns = ["status", "where is my order", "how long", "track"]
    if any(pattern in message_lower for pattern in status_patterns):
        result.update({"intent": "order_status", "confidence": 0.8, "navigate_to": "orders"})
    
    robot_patterns = ["robot", "delivery", "deliveries", "where is the robot"]
    if any(pattern in message_lower for pattern in robot_patterns):
        result.update({"intent": "robot_status", "confidence": 0.8, "navigate_to": "delivery"})
    
    profile_patterns = ["profile", "account", "my information"]
    if any(pattern in message_lower for pattern in profile_patterns):
        result.update({"intent": "profile", "confidence": 0.85, "navigate_to": "profile"})
    
    help_patterns = ["help", "support", "assist", "how do i"]
    if any(pattern in message_lower for pattern in help_patterns):
        result.update({"intent": "help", "confidence": 0.8})
    
    hours_patterns = ["hours", "open", "close", "when", "time"]
    if any(pattern in message_lower for pattern in hours_patterns):
        result.update({"intent": "hours", "confidence": 0.8})
    
    quantity_matches = re.findall(r'(\d+)\s+([a-zA-Z]+)', message_lower)
    if quantity_matches:
        quantities = {item: int(quantity) for quantity, item in quantity_matches}
        result["entities"]["quantity"] = quantities
...
async def generate_response(user_message: str, intent_data: Dict, context: Dict) -> str:
    ...
    try:
        system_prompt = f"""
        You are BaristaBot, the virtual assistant for Neo Cafe.
        Current context:
        - Logged in: {context.get('is_logged_in', False)}
        - Page: {context.get('current_page', 'unknown')}
        - Active order: {context.get('has_active_order', False)}
        - Order items: {', '.join(context.get('order_items', []))}
        Intent: {intent_data.get('intent')}
        
        Provide helpful, friendly responses addressing the intent.
        Keep responses under 150 words and conversational.
        Relevant details:
        - Hours: Weekdays 7am-8pm, Weekends 8am-6pm
        - Location: 123 Coffee Street
        - Robot delivery within 1 mile
        """
...
def get_default_response(intent_data: Dict) -> str:
    responses = {
        "greeting": "👋 Hello! Welcome to Neo Cafe. How can I help you today?",
        "menu": "📋 I'd be happy to show you our menu. What would you like to see?",
        "order": "🛒 Let's create your order! What would you like?",
        "order_status": "📊 Checking your order status...",
        "robot_status": "🤖 Checking robot status...",
        "help": "❓ How can I assist you today?",
        "profile": "👤 Accessing profile information...",
        "hours": "🕒 Open: Weekdays 7am-8pm, Weekends 8am-6pm",
        "navigation": "🧭 Navigating...",
        "casual": "How can I help with your Neo Cafe experience today?"
    }
    return responses.get(intent_data.get("intent", "casual"), responses["casual"])
...
#     cl.user_session.set("voice_state", VoiceState())
#     cl.user_session.set("order_state", OrderState())
#     cl.user_session.set("robot_state", RobotState())
...
#     welcome_msg = "👋 Welcome to Neo Cafe! I'm BaristaBot. How can I help?"
#     await cl.Message(content=welcome_msg).send()
    
#     if current_page == "home":
#         await cl.Message(content="## Quick Actions\n- Browse menu\n- Place order\n- Check status").send()


# Update the start function's session handling
@cl.on_chat_start
async def start():
    global menu_items
    menu_items = await load_menu_data()
    ...
    if hasattr(session, 'url_query'):
        query = session.url_query
        query_dict = urllib.parse.parse_qs(query)
        current_page = query_dict.get('tab', ['home'])[0]
...
async def process_message(message: str):
    ...
    if voice_state and voice_state.is_enabled():
        voice_state.speak_text(response.replace("*", ""))
    
    if intent_data.get("intent") == "menu":
        await show_menu()
    elif intent_data.get("intent") == "order":
        await handle_order_intent(intent_data, order_state)

async def handle_order_intent(intent_data: Dict, order_state: OrderState):
    if "entities" in intent_data:
        items = intent_data["entities"].get("menu_items", [])
        quantities = intent_data["entities"].get("quantity", {})
        
        added = []
        for item in items:
            qty = quantities.get(item, 1)
            order_state.add_item(item, qty)
            added.append(f"{qty}x {item}")
        
        if added:
            await cl.Message(content=f"🛒 Added: {', '.join(added)}").send()
            await display_order_summary(order_state)

async def display_order_summary(order_state: OrderState):
    if order_state.items:
        summary = "## Your Order\n" + "\n".join(
...
# Update the load_menu_data function to use proper path handling
async def load_menu_data():
    try:
        # Get the directory of the current file
        current_dir = os.path.dirname(os.path.abspath(__file__))
        # Navigate to project root and then to data/menu.json
...
async def show_menu(category=None):
    ...
    for cat, items in categories.items():
        await cl.Message(content=f"### {cat}").send()
        for item in items:
            card = f"**{item['name']}** - ${item['price']}\n{item['description']}"
            await cl.Message(content=card).send()

@cl.set_starters
async def set_starters():
    return [
        cl.Starter(label="Show Menu", message="Show menu", icon="📋"),
        cl.Starter(label="Place Order", message="I want to order", icon="☕"),
        cl.Starter(label="Order Status", message="Where's my order?", icon="📦"),
        cl.Starter(label="Help", message="Help", icon="❓")
    ]
